    ...
    public static void CheckAABBs(CubeBehaviour a, CubeBehaviour b)
    {
            ...
            (a.min.z <= b.max.z && a.max.z >= b.min.z))
        {
            ...
            if (!a.contacts.Contains(contactB))
            {
                ...
                {
                    a.gameObject.GetComponent<RigidBody3D>().AddForce("Right");
                    
                }

                if (contactB.face == Vector3.right)
                {
                    a.gameObject.GetComponent<RigidBody3D>().AddForce("Left");
                   
                }

                if (contactB.face == Vector3.forward)
                {
                    a.gameObject.GetComponent<RigidBody3D>().AddForce("Back");
                   
                }

                if (contactB.face == Vector3.back)
                {
                    a.gameObject.GetComponent<RigidBody3D>().AddForce("Forward");
